...
public class CourseService {
    ...
    public void addMemberToCourse(UUID courseId, String targetEmail, String role, String requesterEmail) {
        ...
        CourseMember requesterMember = courseMemberRepository.findById(requesterMemberId).orElseThrow(() -> new RuntimeException("Access Denied: You are not a member of this course"));

        if (!"TEACHER".equalsIgnoreCase(requesterMember.getRole())) {
            throw new RuntimeException("Access Denied: Only teachers can add new members to this course");
        }
    ...
    public List<CourseResponse> getAllCourses() {
        return courseRepository.findAll().stream().map(course -> mapToResponse(course, null)) // Simplified for now, we can fetch creator later
