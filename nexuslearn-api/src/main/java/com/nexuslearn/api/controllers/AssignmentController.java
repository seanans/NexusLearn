...
public class AssignmentController {
    ...
    @PostMapping
    public ResponseEntity<Map<String, UUID>> createAssignment(@PathVariable UUID moduleId, @Valid @RequestBody AssignmentCreateRequest request, @AuthenticationPrincipal CustomUserDetails userDetails) {
    ...
    @GetMapping
    public ResponseEntity<List<AssignmentResponse>> getAssignments(@PathVariable UUID moduleId, @AuthenticationPrincipal CustomUserDetails userDetails) {
        return ResponseEntity.ok(assignmentService.getAssignmentsByModule(moduleId, userDetails.user()));
    }

    @PutMapping("/{assignmentId}")
    public ResponseEntity<Void> updateAssignment(@PathVariable UUID moduleId, @PathVariable UUID assignmentId, @Valid @RequestBody AssignmentUpdateRequest request, @AuthenticationPrincipal CustomUserDetails userDetails) {
        assignmentService.updateAssignment(assignmentId, request, userDetails.user());
        return ResponseEntity.ok().build();
    }

    @DeleteMapping("/{assignmentId}")
    public ResponseEntity<Void> deleteAssignment(@PathVariable UUID moduleId, @PathVariable UUID assignmentId, @AuthenticationPrincipal CustomUserDetails userDetails) {
        assignmentService.deleteAssignment(assignmentId, userDetails.user());
        return ResponseEntity.noContent().build();
    }

    @PatchMapping("/{assignmentId}/publish")
    public ResponseEntity<Void> updatePublishStatus(@PathVariable UUID moduleId, @PathVariable UUID assignmentId, @Valid @RequestBody PublishStatusRequest request, @AuthenticationPrincipal CustomUserDetails userDetails) {
